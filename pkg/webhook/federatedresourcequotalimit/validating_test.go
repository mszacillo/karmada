...
func Test_checkQuotaLimitsExceeded(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			"v1beta1-FlinkDeployment",
			testFederatedResourceQuota,
			2,
			&workv1alpha2.ReplicaRequirements{
				ResourceRequest: normalResourceRequest,
			},
			admission.Allowed(""),
		},
		{
		...
		},
		{
			...
			3,
			&workv1alpha2.ReplicaRequirements{
				ResourceRequest: oversizedResourceRequest,
			},
			admission.Denied("Quota (cpu) exceeded for resource v1beta1-FlinkDeployment: limited to 5, current usage is 1, resource requires 6"),
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if actual := checkQuotaLimitsExceeded(tt.quota, tt.gvk, tt.replicas, tt.replicaRequirements); !reflect.DeepEqual(actual, tt.expect) {
