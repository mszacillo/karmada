...
func (v *ValidatingAdmission) Handle(ctx context.Context, req admission.Request) admission.Response {
	...
	replicas, replicaRequirements, err := v.interpretReplicas(obj)
	if err != nil {
		return admission.Errored(http.StatusBadRequest, err)
	}
	...
	return checkQuotaLimitsExceeded(&quotas.DeepCopy().Items[0], obj.GroupVersionKind().String(), replicas, replicaRequirements)
}

func (v *ValidatingAdmission) interpretReplicas(obj *unstructured.Unstructured) (replica int32, requires *workv1alpha2.ReplicaRequirements, err error) {
...
func checkQuotaLimitsExceeded(quota *policyv1alpha1.FederatedResourceQuota, groupVersionKind string, replicas int32, replicaRequirements *workv1alpha2.ReplicaRequirements) admission.Response {
	...
	for resourceName, resourceQuantity := range replicaRequirements.ResourceRequest {
		...
		if limitExists {
			if usedQuantity.Cmp(limitedQuantity) > 0 {
				...
				return admission.Denied(fmt.Sprintf("Quota (%s) exceeded for resource %s: limited to %s, current usage is %s, resource requires %s",
					resourceName, groupVersionKind, limitedQuantity.String(), usedQuantity.String(), resourceQuantity.String()))
			}
		}
	}
	return admission.Allowed("")
